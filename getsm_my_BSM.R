          ...
          plot = NULL, alarm = FALSE) 
{
  ...
  out$gum.diagnostics <- object$diagnostics
    estMethod <- which(vcov.type == c("none", "none", "ordinary", 
                                      "white", "newey-west"))
    est <- kfs.XT(object$aux$y, mXadj, tol = object$aux$tol,  in.old.param=object$old.param,
                  infomat=object$infomat,infomat.std.err=object$infomat.std.err,
